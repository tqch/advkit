...
class DKNNAttack:
    ...
    def knn_loss(self, l, hidden_repr, nn_repr, n_samples):
        eta = torch.FloatTensor(self.eta).to(self.device)
        l2dist = (
                hidden_repr.reshape(n_samples, 1, -1) - nn_repr.reshape(n_samples, self.m_neighborhood, -1)
        ).pow(2).sum(dim=-1).sqrt()
        return (torch.sigmoid(self.alpha * (l2dist/eta[l] - 1))).mean(dim=1).sum()
    ...
    def generate(
        ...
        for i in range(0, x.size(0), self.batch_size):
            ...
            nn_indices_batch = [
                nn_inds[range(len(nn_inds)), targets_batch, :]
            ...
            if to_attack_batch.numel():
                ...
                x_adv_batch = x_batch.clone().detach()
